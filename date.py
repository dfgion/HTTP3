...
class Date_and_time:
    ...
    def array(self):
        ...
        date = datetime.datetime.now() # Получаем дату
        date = str(date).split(' ') # Получаем элементы, которые потом изменяем для корректного преобразования в Unix(Из элемента дня вычитаем 2, а секунды вообще убираем из даты)
